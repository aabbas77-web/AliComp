...
//---------------------------------------------------------------------------
static inline void ValidCtrCheck(TAliGif *)
{
        new TAliGif(NULL);
}
//---------------------------------------------------------------------------
...
namespace Aligif
{
        ...
        {
                 TComponentClass classes[2] = {__classid(TAliGif),__classid(TAGif)};
                 RegisterComponents("AliSoft", classes, 1);

                 TPropInfo* PropInfo=::GetPropInfo(__typeinfo(TAliGif),"FileName");
                 RegisterPropertyEditor(*PropInfo->PropType,
                                        __classid(TAliGif),"FileName",
...
//AliSoft	return PrepareDC(m_PictureSize.cx,m_PictureSize.cy);
}
//---------------------------------------------------------------------------
void TAGif::UnLoad()
{
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
__fastcall TAliGif::TAliGif(TComponent* Owner)
        : TGraphicControl(Owner)
{
 FGif=new TAGif(Owner);
...
//---------------------------------------------------------------------------
void __fastcall TAliGif::DoTimer(TObject *Sender)
{
   if(FGif->m_arrFrames[nTemp].m_pPicture)
    {
     ///////////////////////////////////////////////////////
     ...
     if(FGif->m_arrFrames[nTemp].m_nDisposal == 3)
      {
       ...
       hMemBM = CreateCompatibleBitmap(m_hMemDC,
       						FGif->m_arrFrames[nTemp].m_frameSize.cx,
       						FGif->m_arrFrames[nTemp].m_frameSize.cy);
       ...
       if(hMemDC && hMemBM)
	{
	 ...
	 BitBlt(hMemDC,0,0,
	 		...
	 		FGif->m_arrFrames[nTemp].m_frameSize.cy,
	 		m_hMemDC,
	 		FGif->m_arrFrames[nTemp].m_frameOffset.cx,
	 		FGif->m_arrFrames[nTemp].m_frameOffset.cy,
	 		SRCCOPY);
	};
      };
     ///////////////////////
     ...
     long hmWidth;
     long hmHeight;
     FGif->m_arrFrames[nTemp].m_pPicture->get_Width(&hmWidth);
     FGif->m_arrFrames[nTemp].m_pPicture->get_Height(&hmHeight);

     if(FGif->m_arrFrames[nTemp].m_pPicture->Render(m_hMemDC,
      	FGif->m_arrFrames[nTemp].m_frameOffset.cx,
      	FGif->m_arrFrames[nTemp].m_frameOffset.cy,
      	FGif->m_arrFrames[nTemp].m_frameSize.cx,
      	FGif->m_arrFrames[nTemp].m_frameSize.cy,
      ...
      {
       ::BitBlt(Canvas->Handle,0,0,FGif->m_PictureSize.cx,FGif->m_PictureSize.cy,
     ...
     // disposal method #2
     if(FGif->m_arrFrames[nTemp].m_nDisposal == 2)
      {
       HBRUSH hBrush = CreateSolidBrush(FGif->m_clrBackground);
       if(hBrush)
	{
	 RECT rect = {
	  	FGif->m_arrFrames[nTemp].m_frameOffset.cx,
	 	FGif->m_arrFrames[nTemp].m_frameOffset.cy,
	 	FGif->m_arrFrames[nTemp].m_frameOffset.cx + FGif->m_arrFrames[nTemp].m_frameSize.cx,
	 	FGif->m_arrFrames[nTemp].m_frameOffset.cy + FGif->m_arrFrames[nTemp].m_frameSize.cy };
	 FillRect(m_hMemDC,&rect,hBrush);
	 DeleteObject(hBrush);
	};
      }
     else
      if(hMemDC && (FGif->m_arrFrames[nTemp].m_nDisposal == 3))
       {
	// put it back
	BitBlt(m_hMemDC,
			FGif->m_arrFrames[nTemp].m_frameOffset.cx,
 ...
 nTemp++;
 if(nTemp >= FGif->m_arrFrames.size())
  {
   nTemp = 0;
   // init the screen for the first frame,
   RECT rect = {0,0,FGif->m_PictureSize.cx,FGif->m_PictureSize.cy};
   FillRect(m_hMemDC,&rect,(HBRUSH)(COLOR_WINDOW));
  };
}
//---------------------------------------------------------------------------
void __fastcall TAliGif::Paint()
{
 if(!FGif->IsGIF())
  {
   Width=64;
   ...
   Canvas->Font->Size=13;
   Canvas->Font->Color=TColor(0x009D0000);
   Canvas->Font->Style=TFontStyles()<<fsBold;
   Canvas->TextOut(5,10,"AliSoft");
   Canvas->Font->Size=15;
   Canvas->Font->Color=TColor(0x00009700);
   Canvas->TextOut(15,30,"GIF");
  }
 else
  {
   if(FGif->m_PictureSize.cx<=0)
    Width=64;
   else
    Width=FGif->m_PictureSize.cx;
   if(FGif->m_PictureSize.cy<=0)
    Height=64;
   else
    Height=FGif->m_PictureSize.cy;
   ::BitBlt(Canvas->Handle,0,0,FGif->m_PictureSize.cx,FGif->m_PictureSize.cy,
            m_hMemDC,0,0,SRCCOPY);
  }
}
//---------------------------------------------------------------------------
void __fastcall TAliGif::SetFileName(AnsiString Value)
{
 UnLoad();
 ...
 //AliSoft 
 PrepareDC(FGif->m_PictureSize.cx,FGif->m_PictureSize.cy);

 Draw();
 FAnimate=true;
}
//---------------------------------------------------------------------------
void __fastcall TAliGif::SetAnimate(bool Value)
{
 if(FGif->IsAnimatedGIF())
  {
   FAnimate=Value;
   ...
   if(FGif->m_pPicture)
    {
     long hmWidth;
     long hmHeight;
     FGif->m_pPicture->get_Width(&hmWidth);
     FGif->m_pPicture->get_Height(&hmHeight);

     if (FGif->m_pPicture->Render(m_hMemDC, 0, 0, FGif->m_PictureSize.cx, FGif->m_PictureSize.cy,
	        	    0, hmHeight, hmWidth, -hmHeight, NULL) == S_OK)
      {
       ::BitBlt(Canvas->Handle,0,0,FGif->m_PictureSize.cx,FGif->m_PictureSize.cy,
                m_hMemDC,0,0,SRCCOPY);
...
//---------------------------------------------------------------------------
bool TAliGif::PrepareDC(int nWidth, int nHeight)
{
 ...
 // fill the background
 FGif->m_clrBackground = GetSysColor(COLOR_3DFACE);
 RECT rect = {0,0,nWidth,nHeight};
 FillRect(m_hMemDC,&rect,(HBRUSH)(COLOR_WINDOW));

 FGif->m_bIsInitialized = TRUE;
 return TRUE;
}
//---------------------------------------------------------------------------
void TAliGif::UnLoad()
...
//---------------------------------------------------------------------------
void TAliGif::SetBkColor(COLORREF clr)
{
 FGif->SetBkColor(clr);
...
//---------------------------------------------------------------------------
void __fastcall  TAliGif::LoadCompProperty(TReader *Reader)
{
 if(Reader->ReadBoolean())
  FGif =(TAGif *)Reader->ReadComponent(NULL);
}
//---------------------------------------------------------------------------
void __fastcall  TAliGif::StoreCompProperty(TWriter *Writer)
{
 if(FGif)
  {
   Writer->WriteBoolean( true );
   Writer->WriteComponent(FGif);
  }
 else
  Writer->WriteBoolean( false );
 }
//---------------------------------------------------------------------------
void __fastcall TAliGif::DefineProperties(TFiler *Filer)
{
/*
 ...
 if (Filer->Ancestor) // check for inherited value
  {
   if((((TAliGif *)(Filer->Ancestor))->FGif)==NULL)
    WriteValue = (FGif != NULL);
   else if ((FGif == NULL) ||
            (((TAliGif *)Filer->Ancestor)->FGif->Name !=
              FGif->Name))
    WriteValue = true;
   else
    WriteValue = false;
  ...
  WriteValue = (FGif != NULL);
 Filer->DefineProperty("FGif",LoadCompProperty,StoreCompProperty, WriteValue);
*/ 
}
//---------------------------------------------------------------------------
//-------------------TFileName-----------------------------------------------
